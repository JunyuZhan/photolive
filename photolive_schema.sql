...
DROP POLICY IF EXISTS "用户只能查看自己的分享链接" ON public.share_links;
DROP POLICY IF EXISTS "用户只能插入自己的分享链接" ON public.share_links;
DROP POLICY IF EXISTS "用户只能更新自己的分享链接" ON public.share_links;
DROP POLICY IF EXISTS "用户只能删除自己的分享链接" ON public.share_links;

-- 按正确的顺序删除表以避免外键约束问题
DROP TABLE IF EXISTS public.album_collaborators;
...
CREATE TABLE public.albums (
    ...
    view_count INT DEFAULT 0,
    download_count INT DEFAULT 0,
    cover_photo_id UUID REFERENCES public.photos ON DELETE SET NULL, -- 封面照片
    event_start TIMESTAMPTZ, -- 活动/直播开始时间
    event_end TIMESTAMPTZ,   -- 活动/直播结束时间
    location TEXT,           -- 活动地点
    is_public BOOLEAN DEFAULT true NOT NULL
);
